...
func (r repo) CreateInfo(user domain.UserInfo) (id int, err error) {
	query := `
	insert into  user_info (user_id,firstname,lastname,skin_color,skin_type,gender,created_at) values($1,$2,$3,$4,$5,$6,$7) RETURNING id
`
	row := r.db.QueryRow(query, user.Id, user.Firstname,  user.Lastname, user.SkinColor,user.SkinType,user.Gender,user.UpdatedAt )
...
func (r repo) GetUserInfo(userId int) (user domain.UserInfo, err error) {
	query := `
	select id, firstname,lastname,skin_color,skin_type,gender from user_info where user_id=$1
	`
